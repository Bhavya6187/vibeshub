...
Two Claude Code wrappers cover the common directions without naming a target:
`/handoff` (Claude Code) uploads the current Claude Code session and imports it
into Codex in one step, and `/import <trace-url-or-short-id> [--checkout]`
(Claude Code) runs this same script with `--to claude` appended, for the trip
back.
